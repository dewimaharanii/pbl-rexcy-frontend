...
import '../../screens/produsen/edit_produksi_screen.dart';
...
class _DaftarProduksiScreenState extends State<DaftarProduksiScreen> {
  ...
  void initState() {
    super.initState();
    // Load data dari API saat halaman pertama kali dibuka
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<ProduksiProvider>().loadProduksi();
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
          ...
          : Column(
              children: [
                Padding(
                  ...
                  child: Row(children: [
                    MetricCard(label: 'Total produksi', value: '${totalKg.toStringAsFixed(0)} kg', accent: true),
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => Navigator.pushNamed(context, '/produsen/input-produksi'),
        backgroundColor: AppColors.primary,
        foregroundColor: Colors.white,
        icon: const Icon(Icons.add),
        label: const Text('Tambah Produksi', style: TextStyle(fontWeight: FontWeight.w500)),
      ),
    );
  }

  void _edit(BuildContext context, ProduksiModel item) {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => EditProduksiScreen(item: item)),
    );
  }
  ...
  void _hapus(BuildContext context, ProduksiModel item) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text('Data "${item.jenisProduk}" (${item.kategori}) pada ${item.tanggal} akan dihapus.',
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              await context.read<ProduksiProvider>().hapus(item.id);
              if (context.mounted) {
    ...
    return AppCard(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Container(
                  ...
                  child: Text(item.kategori,
                      style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: kategoriColor)),
                ),
                ...
                Row(children: [
                  Container(
                      width: 6, height: 6,
                      decoration: const BoxDecoration(color: Color(0xFF288AE7), shape: BoxShape.circle)),
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                        ...
                        borderRadius: BorderRadius.circular(8)),
                    child: const Icon(Icons.edit_outlined, size: 16, color: Color(0xFF288AE7)),
                  ),
                ),
        ...
        Row(children: [
          ...
          Expanded(child: Text(item.lokasiTangkap,
              style: const TextStyle(fontSize: 11, color: AppColors.textSecondary))),
        ]),
